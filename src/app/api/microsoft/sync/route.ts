...
import { refreshAccessToken, listRecentMessages } from "@/lib/microsoft";
...
export async function POST() {
  ...
  if (!account) {
    return NextResponse.json({ error: "Geen Microsoft 365-account gekoppeld" }, { status: 400 });
  }
    ...
    if (Date.now() > expiresAt - 60_000) {
      if (!refreshToken) {
        return NextResponse.json(
          { error: "Token verlopen en geen refresh-token beschikbaar. Koppel Microsoft 365 opnieuw." },
          { status: 400 }
        );
      }
      ...
      await supabase.from("email_accounts").update(updatePayload).eq("id", account.id as string);
    }
